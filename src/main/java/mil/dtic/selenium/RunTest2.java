...
public class RunTest2 {
	...
	public static final Logger logger = Logger.getLogger(RunTest2.class.getName());;
	...
	public void beforeClass() throws IOException {
		...
		// Create a blank sheet
		//sheetTestSteps = workbook.createSheet("Test Steps");
		...
		// Header
		data1.put("1", new Object[] { "Test Suite", "Test Case Number", "Test Step Number", "Test Step Description", "Element", "Value", "Keyword",
	...
	public void readExcel() throws Exception {
		...
		for (int i = 1; i < r; i++) {
			...
			//Input suiteInput = new Input(workbook.createSheet(description), "", executionFlag, new Object[] {serialNumber, description, executionFlag});
			...
			if (executionFlag.equalsIgnoreCase("Y")) {
				...
				// set execution flag to 'Y'
				//suiteInput.setObjArray(new Object[] {serialNumber, description, executionFlag});
						...
						for (int w = 1; w < testStepsRow; w++) {
							...
							if (testCaseNumber.equalsIgnoreCase(testStepCaseNumber)) {
								...
								if (result.isResult() == true) {
									...
									data1.put(Integer.toString(w), objectArray);
									...
									inputs.add(input);
									//writeToExcel(data1, sheetTestSteps);
								} else {
		...
		logger.info("Looping through Test Cases ...");
		for (Input input : caseInputs) {
			logger.info("Sheet : " + input.getSheetName() + " " + input.getExecutionFlag());
			Object[] objArray = input.getObjArray();
			for (Object obj : objArray) {
				logger.info("The value of obj[0] is -> " + obj);
			}
